...
export default function RoomClient({
  ...
  }, [isAdmin, isLive, hasJoined, roomId])
    ...
    return (
      <main className='min-h-screen bg-stone-950 text-stone-100 flex flex-col items-center justify-center gap-6'>
        ...
        <div className='flex items-center gap-3'>
          ...
          <button
            onClick={() => router.push(`/room/${roomId}/customize`)}
        ...
        <div className='flex-1 relative overflow-hidden'>
          ...
          <div className='relative z-10 h-full'>
            <VideoRoom
              ...
              musicTrackKey={
                currentSoundtrack
                  ? `${currentSoundtrack.id}-${currentTrackIndex}`
                  : undefined
              }
            />
          </div>
          ...
          >
            <div className='w-80 flex-1 min-h-0'>
              <DmPanel
                ...
                onProfileUpdated={async (profile: { characterName: string }) => {
                  setDmCharacterName(profile.characterName)
                  await fetch(`/api/rooms/${roomId}/membership`, {
                    method: "PUT",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ characterName: profile.characterName }),
                  })
                }}
                onOpenPlayerManager={() => setShowRoomMembersPanel(true)}
